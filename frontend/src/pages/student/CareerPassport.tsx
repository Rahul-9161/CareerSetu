...
export default function CareerPassport() {
  ...
  useEffect(() => {
    ...
    facultyApi.getEndorsements()
      .then(res => setEndorsements(res))
      .catch(err => console.warn('Failed to load endorsements:', err))
  }, [])
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible" className="max-w-4xl space-y-6 mx-auto">
      ...
      {/* Verified Faculty Recommendations & Academic Endorsements */}
      <motion.div variants={fadeUp} custom={7}>
        <Section title="Verified Faculty Endorsements & Recommendations" icon={ShieldCheck}>
          <div className="space-y-3">
            {endorsements.length === 0 ? (
              <p className="text-xs text-slate-400">No faculty endorsements on record yet.</p>
            ) : (
              endorsements.map(end => (
                <div
                  key={end.id}
                  className="p-4 rounded-xl border space-y-2.5 transition hover:shadow-xs"
                ...
                >
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-[10px] text-slate-400">
                          ({end.facultyDesignation} • {end.facultyDepartment})
                        </span>
                      </div>
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-300 italic font-serif leading-relaxed bg-slate-50 dark:bg-slate-800/40 p-3 rounded-lg border border-slate-100 dark:border-slate-700/50">
                    "{end.endorsementText}"
                  </p>

                  <div className="flex items-center justify-between pt-1 text-[10px] font-mono text-slate-400">
                    <span>Digital SHA-256 Stamp: {end.verificationHash.slice(0, 20)}...{end.verificationHash.slice(-6)}</span>
                    <span className="text-emerald-600 font-sans font-semibold">Immutable On-Chain Stamp</span>
                  </div>
                </div>
              ))
            )}
