  ...
  return (
    <div>
      ...
      <div className="mt-6 p-4 rounded-xl border" style={{ background: 'var(--surface-inset)', borderColor: 'var(--border-light)' }}>
        <p className="text-xs font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>🔧 Demo Accounts (Click to Fill & Test)</p>
        <div className="space-y-1.5">
          {[
            { email: 'student@demo.local', role: 'Student' },
            { email: 'recruiter@demo.local', role: 'Recruiter' },
            { email: 'tpo@demo.local', role: 'TPO' },
            { email: 'faculty@careersetu.in', role: 'Faculty / Academic Mentor' },
            { email: 'alumni@careersetu.in', role: 'Alumni / Referral Partner' },
            { email: 'compliance@careersetu.in', role: 'Compliance / Grievance Officer' },
          ].map(({ email, role }) => (
            <button
              key={email}
              type="button"
              ...
              className="w-full text-left p-1.5 rounded-lg hover:bg-slate-200/50 transition flex items-center justify-between text-xs font-mono"
              style={{ color: 'var(--text-muted)' }}
            >
              <span>{role}: <span style={{ color: 'var(--color-brand-500)' }}>{email}</span></span>
              <span className="text-[10px] text-blue-600 font-sans font-medium">Click to fill</span>
            </button>
          ))}
        </div>
      </div>
